...
import ProductsSort from '../components/ProtuctsSort.js';
...
const Shop = observer(() => {
  ...
  //стейты для мин/макс цены
  const [maxPrice, setMaxPrice] = useState(null);

  //Useeffect, в котором делаем запрос к бд на получение меньшей и большей цены при первом рендере
  useEffect(() => {
  ...
  useEffect(() => {
    const { category, brand, mehanizm, shape, gender, material, glass, strap, power, water, page } =
      getSearchParams(searchParams);
    ...
    <Container>
      ...
      <Row className="mt-4">
        <Col md={3} className="mb-3">
          {maxPrice ? <PriceSlider minPrice={minPrice} maxPrice={maxPrice} /> : null}
        ...
        <Col md={9}>
          <div>{productsFetching ? <Spinner animation="border" /> : <ProductList />}</div>
        </Col>
      </Row>
    </Container>
  );
